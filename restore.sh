...
if [ "${BACKUP_FILE}" = "**None**" ]; then
  ...
  echo "Use S3_PREFIX/filename format. Example: backup/database_0000-00-00T00:00:00Z.sql.xz"
  exit 1
fi
...
if [[ "$LOCAL_FILE" == *.enc ]]; then
  ...
  gpg -d $DOWNLOAD_PATH -o $DECRYPTED_PATH --batch --passphrase $ENCRYPTION_PASSWORD
  if [ $? != 0 ]; then
...
if [[ "$DOWNLOAD_PATH" == *.dump ]]; then
  ...
  echo "ERROR: Unsupported backup format. Expected *.sql.xz or *.dump file."
  exit 1
fi
